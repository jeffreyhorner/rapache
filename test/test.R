...
handler <- function(r)
{
    ...
    cat("<HTML><BODY><H1>Hello from mod_R</H1>\n")
    cat("<form enctype=multipart/form-data method=POST action=\"/test/R?called=",called,"\">\n",sep="")
    cat("Enter a string: <input type=text name=name value=\"",post$name,"\"><br>\n",sep="")
    cat("Enter another string: <input type=text name=name value=\"",post$name,"\"><br>\n",sep="")
    cat("Upload a file: <input type=file name=file><br>\n")
    cat("<input type=submit name=Submit>")

    cat("<hr>\n")
    cat("<h2>Args variables</h2>\n")
    cat(as.html(args));
    cat("<h2>Post variables</h2>\n")
    cat(as.html(post))
    cat("<h2>Cookies</h2>\n")
    cat(as.html(cookies))
    cat("<h2>File Uploads</h2>\n")
    cat(as.html(uploads))
    cat("<h2>Request Record</h2>\n")
    cat(as.html(r))
	cat("<h2>About R</h2>\n")
	cat("R.version: ",as.html(as.list(unlist(R.version))),'<br>',sep="")
	cat("Sys.info(): ",as.html(as.list(Sys.info())),'<br>',sep="")
    cat("Sys.getenv(): ",as.html(as.list(Sys.getenv())),'<br>',sep="")
	cat(".Platform: ",as.html(.Platform),'<br>',sep='')
	cat(".Machine: ",as.html(.Machine),'<br>',sep='')
    cat(".libPaths(): ",as.html(.libPaths()),'<br>',sep="")
	cat("</BODY></HTML>\n")
